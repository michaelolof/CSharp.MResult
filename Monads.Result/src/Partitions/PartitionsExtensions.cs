...
namespace Michaelolof.Monads.Result
{
  ...
  public static class PartitionsExtensions
  {
    ...
    internal static Partitions<V,E> ToPartitions<V,E>(this Result<V,E> result)
    {
      var allVals = new List<V>();
      var allErss = new List<E>();
    ...
    public static IList<V> GetVal<V,E>(this Partitions<V,E> partitions) => partitions.val;
    public static IList<E> GetErr<V,E>(this Partitions<V,E> partitions) => partitions.err;
    public static (IList<V>,IList<E>) GetValAndErr<V,E>(this Partitions<V,E> partitions) => (partitions.GetVal(), partitions.GetErr());
    #endregion
    ...
    public static Partitions<TV,E> OnOk<V,E,TV>(this Partitions<V,E> partition, TV value ) 
    {
      ...
      return (vals.ToList(), partition.GetErr());
    }
    ...
    public static Partitions<TV,E> Then<V,E,TV,VE>(this Partitions<V,E> partitions, Partitions<TV,VE> monad ) where VE : E
    {
      ...
      var errs = partitions.GetErr().ToList();
      var monadErrs = monad.GetErr() as ICollection<E>; 
      errs.AddRange( monadErrs );
      return new Result.Partitions<TV,E>(vals.ToList(), errs );
    }
    ...
    {
      var errs = partition.GetErr().Select( e => err ).ToList();
    ...
    {
      var errs = partition.GetErr().Select( handler ).ToList();
    ...
    public static Partitions<V,TE> Catch<V,E,EV,TE>(this Partitions<V,E> partitions, Partitions<EV,TE> monad ) where V : EV 
    {
      ...
      var vals = partitions.GetVal().ToList();
    ...
    public static IList<T> Merge<T>(this Partitions<T,T> partitions) {
      var val = partitions.GetVal();
      val.ToList().AddRange( partitions.GetErr() );
      return val;
    }

    private static bool Has<T>(IList<T> stuff) => stuff.ToList().Count > 0;
